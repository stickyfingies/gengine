...
namespace gengine
{
	...
	public:
		...
		auto create_capsule(float mass, const float model_matrix[16])->Collidable*;
		auto create_mesh(float mass, const std::vector<float>& vertices, const std::vector<unsigned int>& indices, const float model_matrix[16]) -> Collidable*;
		...
		auto set_velocity_x(Collidable* collidable) -> float;
		auto set_velocity_z(Collidable* collidable) -> float;
