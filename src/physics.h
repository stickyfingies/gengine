...
class PhysicsEngine {
  public:
	...
	auto create_sphere(float const size, float mass,
					   const glm::mat4 &model_matrix) -> Collidable *;
	auto create_capsule(float mass, const glm::mat4 &model_matrix)
		-> Collidable *;
	auto create_mesh(float mass, const std::vector<float> &vertices,
	...
	auto get_model_matrix(Collidable *collidable, glm::mat4 &model_matrix)
		-> void;
